...
- 图书浏览与搜索模块：支持图书分页浏览、分类筛选、关键词检索、查看图书详情。  **[已完成]**
- 图书推荐模块（KNN）：基于用户评分的 KNN 计算，输出个性化推荐、相似图书与猜你喜欢等。  **[已完成-基础]**
- 购物车模块：将图书加入购物车、修改数量、删除商品、清空购物车与统计购物车数量。  **[已完成-基础]**
- 订单管理模块：创建订单、查看订单列表与详情、取消订单、支付订单、确认收货。  **[已完成-基础]**
- 收货地址模块：维护收货地址增删改查与默认地址设置。  **[已完成-基础]**
- 收藏模块：收藏/取消收藏图书，查看收藏列表与收藏状态。  **[已完成-基础]**
- 评价模块：对已购图书进行评价，查看图书评价列表与我的评价。  **[已完成-基础]**
- 浏览记录模块：记录并查看用户浏览历史，支持删除与清空。  **[已完成-基础]**
...
- 图书与推荐
  ...
  - 推荐：个性化/相似/猜你喜欢/热门/新书  **[已完成-基础]**
- 购物流程
  - 购物车管理（增删改查/数量统计）  **[已完成-基础]**
  - 收货地址管理（增删改查/默认地址）  **[已完成-基础]**
  - 订单管理（创建/取消/支付/确认收货/列表/详情）  **[已完成-基础]**
- 互动与记录
  - 图书评价（新增/删除/列表）  **[已完成-基础]**
  - 收藏管理（收藏/取消/列表/检查）  **[已完成-基础]**
  - 浏览记录（列表/删除/清空）  **[已完成-基础]**
...
- 订单管理（列表/详情/发货/状态变更/删除）  **[已完成-基础]**
- 数据统计（总览/销量/排行/分类销量/用户增长）  **[已完成-基础]**
...
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 购物车列表 | GET | /api/cart/list | 已完成 |
| 加入购物车 | POST | /api/cart/add | 已完成 |
| 修改数量 | PUT | /api/cart/update | 已完成 |
| 删除商品 | DELETE | /api/cart/{id} | 已完成 |
| 清空购物车 | DELETE | /api/cart/clear | 已完成 |
| 购物车数量 | GET | /api/cart/count | 已完成 |

### 4.5 收货地址
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 地址列表 | GET | /api/address/list | 已完成 |
| 添加地址 | POST | /api/address/add | 已完成 |
| 更新地址 | PUT | /api/address/update | 已完成 |
| 删除地址 | DELETE | /api/address/{id} | 已完成 |
| 设置默认地址 | PUT | /api/address/default/{id} | 已完成 |

### 4.6 订单
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 创建订单 | POST | /api/order/create | 已完成 |
| 订单列表 | GET | /api/order/list | 已完成 |
| 订单详情 | GET | /api/order/detail/{id} | 已完成 |
| 取消订单 | PUT | /api/order/cancel/{id} | 已完成 |
| 支付订单 | PUT | /api/order/pay/{id} | 已完成 |
| 确认收货 | PUT | /api/order/confirm/{id} | 已完成 |

### 4.7 评价
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 添加评价 | POST | /api/review/add | 已完成-基础 |
| 图书评价列表 | GET | /api/review/book/{bookId} | 已完成-基础 |
| 我的评价 | GET | /api/review/my | 已完成-基础 |
| 删除评价 | DELETE | /api/review/{id} | 已完成-基础 |

### 4.8 收藏
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 添加收藏 | POST | /api/favorite/add/{bookId} | 已完成-基础 |
| 取消收藏 | DELETE | /api/favorite/remove/{bookId} | 已完成-基础 |
| 收藏列表 | GET | /api/favorite/list | 已完成-基础 |
| 是否已收藏 | GET | /api/favorite/check/{bookId} | 已完成-基础 |

### 4.9 浏览记录
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 浏览记录列表 | GET | /api/history/list | 已完成-基础 |
| 清空浏览记录 | DELETE | /api/history/clear | 已完成-基础 |
| 删除单条记录 | DELETE | /api/history/{id} | 已完成-基础 |

### 4.10 文件上传
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 上传图片 | POST | /api/file/upload | 已完成-基础 |
| 上传图书封面 | POST | /api/file/upload/cover | 已完成-基础 |
| 上传用户头像 | POST | /api/file/upload/avatar | 已完成-基础 |

### 4.11 推荐（KNN）
| 功能 | 方法 | 路径 | 状态 |
|---|---|---|---|
| 个性化推荐 | GET | /api/recommend/personal | 已完成-基础 |
| 热门推荐 | GET | /api/recommend/hot | 已完成-基础 |
| 相似图书 | GET | /api/recommend/similar/{bookId} | 已完成-基础 |
| 猜你喜欢 | GET | /api/recommend/guess | 已完成-基础 |
| 新书推荐 | GET | /api/recommend/new | 已完成-基础 |
...
| 订单管理 | GET/PUT/DELETE | /api/admin/order/** | 已完成-接口 |
| 数据统计 | GET | /api/admin/statistics/** | 已完成-基础 |
| 推荐配置 | GET/PUT | /api/admin/recommend/config | 已完成-基础 |
...
## 7. 开发记录（简要）
- 2026-02-10：完成推荐配置模块（K 值/相似度/权重可配置）
- 2026-02-10：完成文件上传模块（头像/封面上传与URL回写）
- 2026-02-10：完成管理员数据统计模块（总览/销量趋势/排行/分类销量/用户增长）
- 2026-02-10：完成 KNN 推荐模块前后端联调（个性化/猜你喜欢/热门/新书/相似图书）
- 2026-02-10：完成浏览记录模块前后端联调（记录/列表/删除/清空）
- 2026-02-09：完成收藏模块前后端联调（收藏/取消/列表/收藏状态）
- 2026-02-09：完成评价模块前后端联调（图书评价、我的评价、删除评价）
- 2026-02-09：完成收货地址模块与下单流程（地址选择 + 创建订单）
- 2026-02-09：完成购物车模块前后端联调（购物车列表/增删改清空）
- 2026-02-09：完成订单模块前后端联调（用户端订单列表/详情、管理员订单管理）
...
- 后续：完善权限/运营设置等模块；完善前后端对接与页面

---

## 8. 下一步开发建议
1. 优先补齐“图书管理 + 图书列表/详情”接口与前端页面（形成可浏览闭环）
2. 再实现“购物车 + 订单 + 地址”形成交易闭环（已完成基础版）
3. 最后完善“权限/运营/系统设置”等模块
...
- 购物车：添加购物车、修改数量、删除商品、清空购物车。**[已完成-基础]**
- 订单管理：创建订单、订单列表、订单详情、取消订单。**[已完成-基础]**
- 下单流程：购物车选择商品 -> 选择收货地址 -> 提交订单。**[已完成-基础]**
- 个人中心：个人信息修改、收货地址管理、购买历史。**[已完成-基础]**
- 图书推荐：基于 KNN 的个性化推荐、热门推荐、猜你喜欢、相似图书。**[已完成-基础]**
- 图书评价：评分、评论、查看评论。**[已完成-基础]**
...
- 订单管理：订单列表、发货、订单状态修改。**[已完成-基础]**
- 数据统计：销售统计、用户统计、图书销量排行。**[已完成-基础]**
- 推荐配置：KNN 算法参数配置。**[已完成-基础]**
